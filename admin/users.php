...
<div class='navCartSpUsers'>
    <div id='navBlock'>
        ...
        <div id='curNav'><a href='users.php' style='text-decoration:none;'>Пользователи</a></div>
...
<?php
    ...
    // if ($order_no) {echo "<div id='rowOrder'><div class='headline'>Заказ № $order_no</div></div>";}
        $id_user = $_SESSION['id'];
        $queryJewAdm = "SELECT * FROM users";
        if ($search) {$queryJewAdm.=" WHERE user_login LIKE '%$search%'";}
        ...
        if ($queryJewAdm!=null) {
         ...
         echo "<script> let numRows = document.getElementById('orderContent');
                               ...
                               function date_new($date_old) {if ($date_old!='') {
                                ...
                                echo "</form>";
                                echo "</div>";
                echo "<div id='rowOrder' class='rowOrderFORUSERS'>";
                                echo "<div id='numberOrder'><b>ID</b></div>";
                                echo "<div id='sumOrder'><b>Имя</b></div>";  
                                echo "<div id='sumOrder'><b>Логин</b></div>";  
                                echo "<div id='sumOrder'><b>Пароль</b></div>"; 
                                echo "<div id='sumOrder'><b>Дата рождения</b></div>";   
                                echo "<div id='sumOrder'><b>Статус</b></div>";
                                echo  "</div>";
            foreach ($queryJewAdm as $users) {if ($users[6]=='blocked') {$user_st = 'разблокировать';}
                                                  else {$user_st = 'заблокировать';}
                                                $birthFromSqlToNormal = date_new($users[4]);
                echo "<div id='rowOrder' class='rowOrderFORUSERS'>";
                                                                    echo "<div id='numberOrder'>$users[0]</div>";
                                                                    echo "<div id='sumOrder'>$users[1]</div>";  
                                                                    echo "<div id='sumOrder'>$users[2]</div>";  
                                                                    echo "<div id='sumOrder'>$users[3]</div>"; 
                                                                    echo "<div id='sumOrder'>$birthFromSqlToNormal</div>";   
                                                                    echo "<div id='sumOrder' class='$users[6]'><a href='blockUser.php?idUser=$users[0]&block=$users[6]' class='$users[6]' style='text-decoration:none;'>$user_st</a></div>";
                                                                    echo  "</div>";} }
