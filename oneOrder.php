...
<div class='navCart'>
    <div id='navBlock'>
        <div id='curNav'><a href='orders.php'>Заказы</a></div>
        <div><a href='settings.php'>Настройки</a></div>
...
if ($order_no) {echo "<div id='rowOrder'><div class='headline'>Заказ № $order_no</div></div>";}
    $id_user = $_SESSION['id'];
    $queryOrders = mysqli_fetch_all(mysqli_query($conn, "SELECT jew_name, jew_img, jew_price FROM orders JOIN item ON orders.order_id=item.item_order JOIN jewelery ON item.item_jew=jewelery.jew_id WHERE item_order=$order_no"));
    if ($queryOrders!=null) {
     $i=2;
     $ii=0;
        foreach ($queryOrders as $orders) {$i++; 
            ...
            echo "<div id='rowOrder' class='rowOrderForORDERS'>";
                                                                echo "<div id='numberOrder'>$ii</div>"; 
                                                                echo "<img src='resources/$orders[1]' id='imgOrder'>"; 
