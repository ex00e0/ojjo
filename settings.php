...
<div class='navCart'>
    <div id='navBlock'>
        <div><a href='orders.php' style='text-decoration:none;'>Заказы</a></div>
        <div  id='curNav'><a href='settings.php' style='text-decoration:none;'>Настройки</a></div>
        <div><a href='cart.php' style='text-decoration:none;'>Корзина</a></div>
    </div>
    <div id="orderContent">
<?php
    $id_user = $_SESSION['id'];
    $queryOrders = mysqli_fetch_all(mysqli_query($conn, "SELECT * FROM users WHERE user_id=$id_user"));
    
    if ($queryOrders!=null) {foreach ($queryOrders as $orders) {echo "<form action='settingsCheck.php' method='post' class='formSub'>";
        echo "<div id='rowOrder' class='rowOrderForSETTINGS'>";
      echo "<label class='labelAcc'><b>Имя</b></label>"; 
      echo "<input value='$orders[1]' name='name' id='name' class='inputSet' required>";
     
      echo  "</div>";
      echo "<div id='rowOrder' class='rowOrderForSETTINGS'>";
        echo "<label class='labelAcc'><b>Логин</b></label>"; 
        echo "<input value='$orders[2]' name='login' id='login' class='inputSet' required>"; 
        echo  "</div>";
       
        echo "<div id='rowOrder' class='rowOrderForSETTINGS'>";
        echo "<label for='password' class='labelAcc'><b>Пароль</b></label>";
        echo "<input value='$orders[3]' name='password' id='password' class='inputSet' required>"; 
        echo  "</div>";
       function date_new($date_old) {$date = date("d.m.Y", strtotime($date_old));
                                        return substr($date, 0, 2).".".substr($date, 3,2).".".substr($date, 6);}
        $birthFromSqlToNormal = date_new($orders[4]);
        
        echo "<div id='rowOrder' class='rowOrderForSETTINGS'>";
       echo "<label for='birth' class='labelAcc'><b>Дата рождения</b></label>";
       echo "<input value='$birthFromSqlToNormal' name='birth' id='birth' class='inputSet'>";    
         echo  "</div>"; 
       
         echo "<div id='rowOrder' class='rowOrderForSETTINGS'>";
       echo "<input type='submit' value='Сохранить' id='submitSet' class='inputSet'>";   
         echo  "</div>";
        echo "</form>"; }      } 
