...
<div class='navCart'>
    <div id='navBlock'>
        ...
        <div><a href='settings.php' style='text-decoration:none;'>Настройки</a></div>
        <div id='curNav'><a href='cart.php' style='text-decoration:none;'>Корзина</a></div>
    </div>
    <div id="orderContent">
<?php
    $id_user = $_SESSION['id'];
    if (isset($_SESSION['arrItems'])) {echo "<div id='rowOrder' class='rowOrderForORDERS'>";
        echo "<div id='numberOrder'><b>№ заказа</b></div>"; 
        echo "<div id='dateOrder'><b>Дата заказа</b></div>"; 
        echo "<div id='getOrder'><b>Дата получения</b></div>";
        echo "<div id='sumOrder'><b>Сумма заказа</b></div>"; 
        echo "<div id='voidOrder'><b></b></div>";   
        echo  "</div>";
     $i=1;
     if (isset($_SESSION['arrItems'])) {$session_arr = $_SESSION['arrItems'];
        for ($r=0;$r<count($session_arr);$r++) {$i++; 
            echo "<script> let numRows = document.getElementById('orderContent');
                           numRows.style.gridTemplateRows='repeat($i,2.5vmax)'; </script>";
            echo "<div id='rowOrder' class='rowOrderForORDERS'>";
            $id_item = $session_arr[$r];
            $queryOrders = mysqli_fetch_all(mysqli_query($conn, "SELECT * FROM jewelery WHERE jew_id=$id_item"));
            foreach ($queryOrders as $orders){
                                                                echo "<div id='numberOrder'>$orders[0]</div>"; 
                                                                echo "<div id='dateOrder'>$orders[2]</div>"; 
                                                                echo "<div id='getOrder'>$orders[3]</div>";
                                                                echo "<div id='sumOrder'>$orders[4] ₽</div>";   
                                                                echo "<div id='voidOrder'><b><a href='oneOrder.php?order=$orders[0]'>Подробнее..</a></b></div>";
                                                                echo  "</div>";}} }}
